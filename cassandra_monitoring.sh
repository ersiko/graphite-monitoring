...
data="$data `curl http://$host:8081/mbean?objectname=org.apache.cassandra.request%3Atype%3DReadStage -s |egrep "CompletedTasks|PendingTasks|ActiveCount"|cut -d">" -f8|cut -d"<" -f1|awk -v now=$now -v host=$host '(NR == 1) {printf("$tree.%s.cassandra.ReadStage.ActiveCount %s %s\\\n",host, $0, now)} (NR == 2) {printf("$tree.%s.cassandra.ReadStage.CompletedTasks %s %s\\\n",host, $0, now)} (NR ==3) {printf("$tree.%s.cassandra.ReadStage.PendingTasks %s %s\\\n",host, $0, now)}'`"

#Tasks in MutationStage - Tasques en MutationStage (writes)
data="$data `curl http://$host:8081/mbean?objectname=org.apache.cassandra.request%3Atype%3DMutationStage -s |egrep "CompletedTasks|PendingTasks|ActiveCount"|cut -d">" -f8|cut -d"<" -f1|awk -v now=$now -v host=$host '(NR == 1) {printf("$tree.%s.cassandra.MutationStage.ActiveCount %s %s\\\n",host, $0, now)} (NR == 2) {printf("$tree.%s.cassandra.MutationStage.CompletedTasks %s %s\\\n",host, $0, now)} (NR ==3) {printf("$tree.%s.cassandra.MutationStage.PendingTasks %s %s\\\n",host, $0, now)}'`"

#Tasks in GossipStage - Tasques en GossipStage (comunicacio interna entre cassandras)
data="$data `curl http://$host:8081/mbean?objectname=org.apache.cassandra.internal%3Atype%3DGossipStage -s |egrep "CompletedTasks|PendingTasks|ActiveCount"|cut -d">" -f8|cut -d"<" -f1|awk -v now=$now -v host=$host '(NR == 1) {printf("$tree.%s.cassandra.GossipStage.ActiveCount %s %s\\\n",host, $0, now)} (NR == 2) {printf("$tree.%s.cassandra.GossipStage.CompletedTasks %s %s\\\n",host, $0, now)} (NR ==3) {printf("$tree.%s.cassandra.GossipStage.PendingTasks %s %s\\\n",host, $0, now)}'`"

#Compaction tasks - Tasques de compactacio
data="$data `curl http://$host:8081/mbean?objectname=org.apache.cassandra.db%3Atype%3DCompactionManager -s|egrep "CompletedTasks|PendingTasks"|cut -d">" -f8|cut -d"<" -f1|awk -v now=$now -v host=$host '(NR == 1) {printf("$tree.%s.cassandra.Compaction.CompletedTasks %s %s\\\n",host, $0, now)} (NR == 2) {printf("$tree.%s.cassandra.Compaction.PendingTasks %s %s\\\n",host, $0, now)}'`"

#Operation Latency - Latencia d'operacions
data="$data `curl http://$host:8081/mbean?objectname=org.apache.cassandra.db%3Atype%3DStorageProxy -s |egrep "RecentRangeLatencyMicros|RecentReadLatencyMicros|RecentWriteLatencyMicros"|cut -d">" -f8|cut -d"<" -f1|awk -v now=$now -v host=$host '(NR == 1) {printf("$tree.%s.cassandra.Latency.Range %s %s\\\n",host, $0, now)} (NR == 2) {printf("$tree.%s.cassandra.Latency.Read %s %s\\\n",host, $0, now)} (NR ==3) {printf("$tree.%s.cassandra.Latency.Write %s %s\\\n",host, $0, now)}'`"
